...
export default function BrandingPage() {
    ...
    async function load() {
      try {
        ...
        if (!res.ok) {
          ...
          setError(`Failed to load branding settings (${res.status})`);
          setLoading(false);
          return;
        }
  ...
  if (loading) {
    return <p style={{ color: 'var(--text-secondary)' }}>Loading branding settings...</p>;
  }

  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        <h1 style={{ fontSize: '1.5rem' }}>Branding Settings</h1>
